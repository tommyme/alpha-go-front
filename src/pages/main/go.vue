<template>
  <q-page class="flex justify-evenly items-center">
    <!-- 远程卡片 -->
    <q-card class="q-ma-md sgf-load-card" v-if="remoteMode">
      <q-card-section>
...
window.rrr = router
const remoteMode = router.currentRoute.value.query.remote === '1'
const modeOptions = ['自我博弈模式', '棋谱阅读模式']
const currentMode = ref('') // ['自我博弈模式', '棋谱阅读模式']
...
const loadRemoteSgfList = () => {
  api
    .get('/stateless/sgf/list')
    .then((resp) => resp.data)
    .then((data) => (remoteSgfList.value = data))
}
...
watch(
  () => router.currentRoute.value.query.remote,
  (a, b) => {
    console.log(a, b)
    if (a != b) {
      location.reload()
